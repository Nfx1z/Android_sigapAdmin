...
export class CloudTasksClient {
    ...
    constructor(opts, gaxInstance) {
        ...
        this._defaults = this._gaxGrpc.constructSettings('google.cloud.tasks.v2beta2.CloudTasks', gapicConfig, opts.clientConfig || {}, { 'x-goog-api-client': clientHeader.join(' ') });
    ...
    initialize() {
        ...
        this.cloudTasksStub = this._gaxGrpc.createStub(this._opts.fallback
            ? this._protos.lookupService('google.cloud.tasks.v2beta2.CloudTasks')
        ...
        for (const methodName of cloudTasksStubMethods) {
            const callPromise = this.cloudTasksStub.then((stub) => (...args) => {
                if (this._terminated) {
    ...
    uploadQueueYaml(request, optionsOrCallback, callback) {
        ...
        const wrappedCallback = callback
            ? (error, response, options, rawResponse) => {
                this._log.info('uploadQueueYaml response %j', response);
                callback(error, response, options, rawResponse);
            }
            : undefined;
        return this.innerApiCalls
            .uploadQueueYaml(request, options, wrappedCallback)
            ?.then(([response, options, rawResponse]) => {
            this._log.info('uploadQueueYaml response %j', response);
            return [response, options, rawResponse];
        })
    ...
    leaseTasks(request, optionsOrCallback, callback) {
        ...
        const wrappedCallback = callback
            ? (error, response, options, rawResponse) => {
                this._log.info('leaseTasks response %j', response);
                callback(error, response, options, rawResponse);
            }
            : undefined;
        return this.innerApiCalls
            .leaseTasks(request, options, wrappedCallback)
            ?.then(([response, options, rawResponse]) => {
            this._log.info('leaseTasks response %j', response);
            return [response, options, rawResponse];
        })
    ...
    acknowledgeTask(request, optionsOrCallback, callback) {
        ...
        const wrappedCallback = callback
            ? (error, response, options, rawResponse) => {
                this._log.info('acknowledgeTask response %j', response);
                callback(error, response, options, rawResponse);
            }
            : undefined;
        return this.innerApiCalls
            .acknowledgeTask(request, options, wrappedCallback)
            ?.then(([response, options, rawResponse]) => {
            this._log.info('acknowledgeTask response %j', response);
            return [response, options, rawResponse];
        })
    ...
    renewLease(request, optionsOrCallback, callback) {
        ...
        const wrappedCallback = callback
            ? (error, response, options, rawResponse) => {
                this._log.info('renewLease response %j', response);
                callback(error, response, options, rawResponse);
            }
            : undefined;
        return this.innerApiCalls
            .renewLease(request, options, wrappedCallback)
            ?.then(([response, options, rawResponse]) => {
            this._log.info('renewLease response %j', response);
            return [response, options, rawResponse];
        })
    ...
    cancelLease(request, optionsOrCallback, callback) {
        ...
        const wrappedCallback = callback
            ? (error, response, options, rawResponse) => {
                this._log.info('cancelLease response %j', response);
                callback(error, response, options, rawResponse);
            }
            : undefined;
        return this.innerApiCalls
            .cancelLease(request, options, wrappedCallback)
            ?.then(([response, options, rawResponse]) => {
            this._log.info('cancelLease response %j', response);
            return [response, options, rawResponse];
        })
